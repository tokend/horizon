...
func NewRequestDetails(request history2.ReviewableRequest) regources.Resource {
	...
	case xdr.ReviewableRequestTypeUpdateKyc:
		return newUpdateKYCRequest(request.ID, *request.Details.UpdateKYC)
...
func newUpdateKYCRequest(id int64, details history2.UpdateKYCRequest) *regources.UpdateKYCRequest {
	return &regources.UpdateKYCRequest{
		Key: regources.NewKeyInt64(id, regources.TypeRequestDetailsUpdateKYC),
		Attributes: regources.UpdateKYCRequestAttrs{
			AccountTypeToSet: details.AccountTypeToSet,
			KYCLevel:         details.KYCLevel,
			KYCData:          details.KYCData,
			SequenceNumber:   details.SequenceNumber,
			ExternalDetails:  details.ExternalDetails,
		},
		Relationships: regources.UpdateKYCRequestRelations{
			AccountToUpdateKYC: NewAccountKey(details.AccountToUpdateKYC).AsRelation(),
		},
	}
}
