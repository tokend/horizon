...
type RequestHelper interface {
	...
	GetWithSignature(string, string, string, string, ...func(*http.Request)) *httptest.ResponseRecorder
}
...
func (rh *requestHelper) GetWithSignature(
	path string,
	timestamp string,
	publicKey string,
	signature string,
	...
	req, _ := http.NewRequest("GET", path, nil)
	req.Header.Set("X-AuthValidUnTillTimestamp", timestamp)
	req.Header.Set("X-AuthPublicKey", publicKey)
	req.Header.Set("X-AuthSignature", signature)
	return rh.Execute(req, mods)
}
